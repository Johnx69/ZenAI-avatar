...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="If set, save a checkpoint every N full epochs (overrides --checkpointing_steps).",
    )
    parser.add_argument(
    ...
    type=int,
    default=10,
    help="Save a checkpoint every N epochs (overrides --checkpointing_steps).",
    )
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if (
            args.checkpointing_epochs          # user asked for epoch saves
            and epoch > 0                      # don’t save at epoch 0
            and epoch % args.checkpointing_epochs == 0
            and accelerator.is_main_process    # one copy only
        ):
            save_path = os.path.join(args.output_dir, f"checkpoint-epoch-{epoch}")
            accelerator.save_state(save_path)
            logger.info(f"[epoch {epoch}] saved checkpoint to {save_path}")
